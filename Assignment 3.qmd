...
qqnorm(residuals_H1)
qqline(residuals_H1, col = "red")

# Step 7: Mixed-effects model to account for non-independence of observations
# Random intercept for id (subject), fixed effect for tipm_e (extraversion), and factor(day)
H1_mixed <- lmer(swl ~ tipm_e + factor(day) + (1 | id), data = A4)
...
residuals_H1_mixed <- residuals(H1_mixed)
plot(residuals_H1_mixed)
...
# Step 7: Check homoscedasticity (Residuals vs Fitted Plot)
# Plot residuals against fitted values to check for constant variance
plot(fitted(H2), residuals_H2)
abline(h = 0, col = "red")

# Step 8: Mixed-effects model to account for non-independence of observations
# Random intercept for id (subject), fixed effect for tipm_n (new measure), and factor(day)
H2_mixed <- lmer(swl ~ tipm_n + factor(day) + (1 | id), data = A4)

# Step 9: Display the summary of the mixed-effects model
summary(H2_mixed)

# Step 10: Check residuals of the mixed-effects model
# Plot residuals of the mixed-effects model to check for patterns or outliers
residuals_H2_mixed <- residuals(H2_mixed)
plot(residuals_H2_mixed)

# Step 11: Check random effects
# Random intercept variance for id
ranef(H2_mixed)

# Step 12: Check confidence intervals for mixed model coefficients
# To get confidence intervals for the fixed effects
confint(H2_mixed)

# Step 13: Intraclass Correlation (ICC) to see the proportion of variance explained by random effects
library(performance)
icc(H2_mixed)

# Step 14: R-squared for the mixed-effects model (marginal and conditional R²)
r2(H2_mixed)
...
H3 <- lmer(swl ~ tipm_e + tipm_n + (1 | id), data = A4)

# Step 2: Visualize the residuals for the model
visualize(H3, plot = "residuals")

# Step 2.5: Checking for multicolinearity
lm_model <- lm(swl ~ tipm_e + tipm_n, data = A4)
...
-- Multicolinearity was assessed using vif found that both tipm_e and tipm_n have low multicolinearity (1.124797 and 1.124797, respectively). Any value below 5 is considered to mean the assumption is met and given that our values are ~1 we can say this assumption is met. MULTICOLINEARITY assumption is met. 

NEUROTICISM
- The estimate for tipm_n is -0.23, indicating a moderate, negative association with satisfaction with life.

- The 95% confidence interval for tipm_n is [-0.25, -0.21], which means we are 95% confident that the true relationship between neuroticism and life satisfaction lies within this range. Since the interval does not include 0, we can confidently conclude that neuroticism is negatively associated with life satisfaction.

-- Individuals higher in neuroticism tend to report lower satisfaction with life.

EXTRAVERSION
- The estimate for tipm_e is 0.16, indicating a moderate, positive association with satisfaction with life.

- The 95% confidence interval for tipm_e is [0.13, 0.18], meaning we are 95% confident that the true relationship between extraversion and life satisfaction lies within this range. Since the interval does not include 0, we can confidently conclude that extraversion is positively associated with life satisfaction.
